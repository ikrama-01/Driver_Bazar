...
# firebase_app = initialize_app(cred, {'databaseURL': 'https://console.firebase.google.com/u/4/project/driverbazar-543a6/database/driverbazar-543a6-default-rtdb/data.com'})
...
# # Real-time user Location into Firebase
...
#         new_location_ref = ref.push(location_data)

#         return jsonify({"success": True, "message": "Location stored successfully", "location_id": new_location_ref.key})
#     else:
...
def check_id_match():
    try:
        ...
        user_id = data.get('id')
        print(user_id)
        # Print the received user ID
        print("Received user ID from frontend:", user_id)
        ...
        if user_id:
            ...
            if matching_driver:
                response = {"match": True}
                # Print the response being sent
                print("Sending response to frontend:", response)
