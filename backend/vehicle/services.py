...
class Vehicle():
  ...
  def readVehicle(self):
    try:
      ...
      for vehicle in data:
        try:
          vehicle['owner'] = db.users.find_one(ObjectId(vehicle['ownerId'])) or db.Drivers.find_one(ObjectId(vehicle['ownerId']))
          vehicle['driver'] = db.Drivers.find_one(ObjectId(vehicle['driverId']))
